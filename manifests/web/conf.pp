...
class ganglia::web::conf (
    ...
    $ldap_auth = true,
    $ldap_group = '',
    $ldap_servers = inline_template('<%= scope.function_hiera(["ldap::uri"]).map{|x| x = x.gsub(/ldaps?:\/\//,"") } %>'),
    $ldap_search = inline_template('ou=People,<%= scope.function_hiera(["ldap::base_dn"]) %>'),
